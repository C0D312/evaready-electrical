...
| Google rating/review count | Live configuration required | `View current Google reviews` outside the live widget | Rating and count are displayed only when returned by the live Google Places widget. Static trust links do not publish a historical number. | Owner-approved EVAREADY ELECTRICAL Place ID and a restricted Google Maps browser key. | Live Google rating widgets; footer Google reviews link. | None. `Review` and `AggregateRating` schema are intentionally not published. |
