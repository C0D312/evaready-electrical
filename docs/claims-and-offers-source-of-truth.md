...
| Google rating/review count | External approval required | `View current Google reviews` outside the live widget | Rating and count are displayed only when returned through an owner-authorised Google Business Profile API summary. Static trust links do not publish a historical number. The public site uses no Maps or Places browser credentials. | Approved Business Profile API access and a successful private OAuth sync for the exact owner-managed EVAREADY ELECTRICAL profile. | Live Google rating widgets; footer Google reviews link. | None. `Review` and `AggregateRating` schema are intentionally not published. |
