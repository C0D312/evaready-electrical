...
test("core-page historical isolation and current authorised register deltas remain independently protected", () => {
  ...
  const others = historicalRegister("publication").records.filter(row => !selected.has(row.route));
