...
test("the deeper editorial ledger accounts for all suburbs without promoting unpublished content", () => {
  ...
  assert.deepEqual(ledger.counts, { routes: 873, reviewed: 140, editorialPending: 733, originalityPassed: 140, unpublishedNewVersions: 140 });
  assert.equal(ledger.method, audit.method);
  assert.equal(ledger.target, 30);
  assert.equal(ledger.minimum, 25);
  assert.deepEqual(ledger.originalitySummary, { bothMinimums: 140, bothTargets: 140, acceptedUnderTolerance: 0 });
